...
async function createApp(){const appId=$('newAppId').value.trim();const name=$('newAppName').value.trim();if(!appId)return setStatus('appStatus','App ID wajib.','warn');await api('/apps',{method:'POST',body:JSON.stringify({appId,name})});await loadApps();setStatus('appStatus','App tersimpan.','ok');}
...
async function saveWebhookSetting(){if(!state.selectedApp)return setStatus('appStatus','Pilih app dulu.','warn');const payload={enabled:$('webhookEnabled').value==='1',url:$('webhookUrl').value.trim(),secret:$('webhookSecret').value.trim()};if(payload.enabled&&!payload.url)return setStatus('appStatus','Webhook URL wajib saat aktif.','warn');await api(`/apps/${state.selectedApp.appId}/webhook-settings`,{method:'PUT',body:JSON.stringify(payload)});await loadApps();setStatus('appStatus','Setting webhook disimpan.','ok');}
